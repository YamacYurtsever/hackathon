...
function ProposalCard({
  ...
  return (
    <div className={keeping ? '' : 'opacity-50'}>
      ...
      />
      {quote && (
        <p className="text-muted-foreground mt-1 ml-3 text-xs">“{quote}”</p>
      )}
    </div>
  )
}
      ...
      <DialogContent className="flex max-h-[85svh] flex-col overflow-hidden sm:max-w-2xl">
        <DialogHeader>
          ...
          <DialogDescription>
            ...
            recorded yet. Drop anything we got wrong, then send the rest for
            review. An admin still has to merge them.
          </DialogDescription>
        </DialogHeader>

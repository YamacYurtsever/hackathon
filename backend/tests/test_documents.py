...
def test_a_submitted_document_fact_says_where_it_came_from(client, project):
    """Checking one proposal at gate 2 shouldn't mean re-reading the document."""
    [request] = client.post(
        f"/api/projects/{project}/requests",
        json={
            "text": "spec.md",
            "operations": [
                {
...
def test_a_typed_message_still_carries_its_own_text(client, project):
    [request] = client.post(
        f"/api/projects/{project}/requests",
        json={
